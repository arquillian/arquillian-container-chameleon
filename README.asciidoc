...
- name: WildFly <1>
  ...
  dist: <15>
    gav: org.wildfly:wildfly-dist:zip:${version} <16>
  defaultProtocol: Servlet 3.0 <17>
  exclude: <18>
    - org.jboss.arquillian.test:* <19>
...
<15> *optional* Section to describe how to download the distribution.
<16> The distribution artifact _GAV_ so it can be downloaded from a repository.
<17> *optional* Override the Adapters _defaultProtocol_ as described by the _DeployableContainer_ implementation.
<18> *optional* List of dependencies to exclude when resolving the adapter _GAV_.
<19> The dependency _GAV_ expression to exclude.
