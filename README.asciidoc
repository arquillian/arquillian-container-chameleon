...
[IMPORTANT]
====
There some limitations when using this approach.

* The first one is that test execution that occurs in the same JVM must use the same container, you cannot run in the same JVM a list of tests that require different containers (i.e some with Wildfly and others with Payara).
